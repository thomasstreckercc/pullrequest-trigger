...
public class PullRequestTriggerResource {
	...
	public Response getBambooInstances(@Context ContainerRequest containerRequest) {
		Iterable<ApplicationLink> bambooInstances = applicationLinkService.getApplicationLinks(BambooApplicationType.class);
		List<RestBambooInstance> instances = new ArrayList<>();
		bambooInstances.forEach(instance -> instances.add(new RestBambooInstance(instance.getId().get(), instance.getName())));
		...
		try {
			return bamboo.createAuthenticatedRequestFactory()
					...
					.executeAndReturn(response -> {
						...
						return ImmutableList.copyOf(Lists.transform(extractProjects(responseObject), RestBambooProject.REST_TRANSFORM::apply));
					});
